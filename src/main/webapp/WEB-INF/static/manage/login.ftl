...
<body class="login-body">
	...
	<div class="container">
		...
		<form class="form-signin" id="adminForm"
			action="${BASE_PATH}/manage/user/login.json" autocomplete="off"
			method="post">
			...
			<div class="login-wrap">
				<div class="form-group">
                                      ...
                                      <input type="text" name="account" class="form-control" placeholder="用户名" value="" style="*width: 250px;" autofocus>
                                  </div>
 				<div class="form-group">
				...
				<div class="form-group">
					<input type="text" name="captcha" class="form-control"
						...
						onclick="this.src='${BASE_PATH}/manage/user/captcha.htm?'+Math.random();"
						src="${BASE_PATH}/manage/user/captcha.htm">
				</div>
					...
					.ajaxForm(
							{
								...
								success : function(data) {
									if (data.result) {
										location.href = "${BASE_PATH}/manage/user/manage.htm";
									} else {
										showErrors($('#adminForm'), data.errors);
										if (data.msg == "change_captcha") {
											$('#captcha').attr(
													"src",
													"${BASE_PATH}/manage/user/captcha.htm?"
															+ Math.random());
											$(
